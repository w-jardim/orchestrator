...
export const containersService = {
  async list(all = true): Promise<Container[]> {
    const { data } = await api.get<ApiResponse<Container[]>>('/api/v1/docker/containers', {
      params: { all },
    })
    return data.data
  },
  ...
  async inspect(id: string): Promise<ContainerInspect> {
    const { data } = await api.get<ApiResponse<ContainerInspect>>(`/api/v1/docker/containers/${id}`)
    return data.data
  },

  async logs(id: string): Promise<string> {
    const { data } = await api.get<ApiResponse<string>>(`/api/v1/docker/containers/${id}/logs`)
    return data.data
  },
